...
pub fn get_source(file: &PathBuf) -> Result<String, Box<dyn Error>> {
    let source = fs::read_to_string(file)?;
    Ok(source)
}

pub fn path_to_str(file: &PathBuf) -> Result<&str, Box<dyn Error>> {
...
pub fn get_stem(file: &PathBuf) -> Result<&str, Box<dyn Error>> {
...
pub fn replace_last<S: AsRef<str>>(file: &PathBuf, replacement: S) -> Result<PathBuf, Box<dyn Error>> {
    let parent = file
        .parent()
